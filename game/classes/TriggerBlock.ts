...
export class TriggerBlock extends Tile /*lib.flash.display.MovieClip*/ {
  ...
  public constructor() {
    ...
    if (
      (this.typeTrigger.includes("DEL") && this.typeTrigger.includes("2")) ||
      this.typeTrigger.includes("POP") ||
      this.typeTrigger.includes("SHW") ||
      this.typeTrigger.includes("JMP")
    ) {
      super.init();
  ...
  public init(): any {
    if (this.typeTrigger.includes("DEL")) {
      ...
      if (this.typeTrigger.includes("1")) {
        for (let i of this.parent.__children) {
  ...
  public addText() {
    ...
    if (this.name.includes("SHW")) this.txt.text = "S" + this.name[4];
    else if (this.name.includes("DEL")) this.txt.text = "D" + this.name[4];
    else if (this.name.includes("SKN")) this.txt.text = " ? ";
    this.txt.x += 3;
    this.txt.y += 5;
  ...
  public triggerDEL(level) {
    ...
    if (this.typeTrigger.includes("DEL")) {
      for (let i: any = 0; i < level.triggers.length; i++) {
        const triggerSrc = level.triggers[i];
